...
from google.adk import Runner  # FIXED: import Runner from google.adk
...
class FinancialTerm(BaseModel):
    ...
    description: str = Field(
        description="Description of what this amount represents"
    )
    due_date: str | None = Field(
        description="When payment is due", default=None
    )
...
class LegalExtraction(BaseModel):
    ...
    parties: list[str] = Field(
        description="All parties involved in the document"
    )
    ...
    governing_law: str = Field(
        description="Jurisdiction or governing law", default=""
    )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(sample_document_path):
        ...
        print("Please ensure the sample_contract.txt file exists in the current directory.")
    else:
